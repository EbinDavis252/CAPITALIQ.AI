...
import plotly.figure_factory as ff # Added for Gantt
from sklearn.ensemble import RandomForestRegressor
import pulp
import sqlite3 # Added for Database
import google.generativeai as genai # Added for Real AI

# ----------------------------------------------------
# 1. Page Configuration & CSS (ORIGINAL RESTORED)
# ----------------------------------------------------
st.set_page_config(
...
# ----------------------------------------------------
# 2. Database & Auth Layer (NEW FEATURES MERGED)
# ----------------------------------------------------
...
def save_scenario_db(name, budget, npv, roi, count, wacc):
    """Saves scenario to SQLite instead of Session State."""
    conn = sqlite3.connect(DB_FILE)
    c = conn.cursor()
...
def standardize_columns(df):
    ...
    column_map = {
        ...
        'Mandatory': 'Is_Mandatory', 'Group': 'Exclusion_Group' # NEW: Added for constraints
    }
    df = df.rename(columns=column_map)
    
    # Ensure Advanced Columns exist (Fill 0 if not present)
...
def get_templates():
    ...
    df_p = pd.DataFrame({
        ...
        "Is_Mandatory": np.random.choice([0, 1], 20, p=[0.9, 0.1]), # NEW
        "Exclusion_Group": 0 # NEW
    })
    return df_h, df_p
...
def run_advanced_optimization(df, budget, min_dept_alloc_pct=0.0):
    """MERGED: Updated with Mandatory and Mutually Exclusive constraints."""
    ...
    # 3. Department Constraints (ORIGINAL)
...
def generate_ai_memo_text(row, api_key):
    """MERGED: Real AI Integration via Google Gemini"""
    if not api_key:
        # Fallback to original template if no key
...
def generate_benchmark_radar(df_prop):
    """2D Radar Chart for AI Insights - Comparing Portfolio to Ideal."""
...
# --- PDF GENERATOR (ORIGINAL RESTORED) ---
try:
...
with st.sidebar:
    ...
    # Original Pages List maintained
    ...
    # MERGED: Replaced simple WACC slider with Professional CAPM Calculator
    with st.expander("WACC Builder (CAPM)"):
        ...
        wacc_input = wacc_calc # Overwrites the old slider value

    budget_input = st.number_input("Budget (INR)", value=15000000.0, step=500000.0)
    # wacc_input was handled above
...
# --- PAGE: EXECUTIVE SUMMARY (ORIGINAL) ---
...
# --- PAGE: EFFICIENT FRONTIER (ORIGINAL) ---
...
# --- PAGE: OPTIMIZATION REPORT (ORIGINAL) ---
...
# --- PAGE: STRATEGIC 3D MAP (ORIGINAL) ---
...
# --- PAGE: SCENARIO MANAGER (MERGED: SQL PERSISTENCE) ---
    ...
    with col_save:
        ...
        # MERGED: Save to DB instead of Session State
    ...
    with col_view:
        # MERGED: Fetch from DB
        df_scenarios = get_scenarios_db()
        
        if not df_scenarios.empty:
            # 1. Chart
            fig_comp = generate_combo_scenario_chart(df_scenarios.to_dict('records')) # Convert back to list for existing func
...
# --- PAGE: AI DEAL MEMOS (MERGED: GEMINI SUPPORT) ---
